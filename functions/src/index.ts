            ...
            line_items: [
              {
                ...
                description: `Liability ID: ${liabilityId} | Destination: ${
                  destination === "deans_office"
                    ? "Dean's Office"
                    : destination === "both"
...
const verifyPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const candidate of sessionCandidates) {
      try {
        response = await axios.get(
          `https://api.paymongo.com/v1/checkout_sessions/${candidate}`,
          {
            headers: {
              Authorization: authHeader,
    ...
    const hasPaidPayment = paymongoPayments.some((p: any) => p?.attributes?.status === "paid");
